...
import type { ActivitySummary, ApplicationStatus, ApplicationType, PointApplication } from "../../api/types";
...
function applicationSummary(app: PointApplication, activityTitles: Map<number, string>): string {
  if (app.type === "type1") {
    const title = app.activityId ? activityTitles.get(app.activityId) : undefined;
    return title ?? "活动心得";
  }
...
export function MeApplicationsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((app) => {
      ...
      if (q) {
        const summary = applicationSummary(app, activityTitles).toLowerCase();
        const no = formatApplicationNo(app.id).toLowerCase();
        if (!summary.includes(q) && !no.includes(q)) return false;
      }
      return true;
    });
  }, [items, status, type, keyword, activityTitles]);
    ...
    <>
      ...
      <div className={shared.filters}>
        ...
        <div className={shared.field}>
          ...
          >
            <option value="all">全部</option>
            <option value="type1">活动心得</option>
            <option value="type2">专项申请</option>
          </select>
        </div>
            ...
            <tbody>
              {filtered.map((app) => (
                <tr key={app.id}>
                  ...
                  <td>{applicationSummary(app, activityTitles)}</td>
