...
登录页使用 **账号 + 密码**；三组演示账号共用密码 **`Demo1234!`**：

| 角色 | 账号 | 密码 | 入口 |
|------|------|------|------|
| 雏鹰 | `eagle@demo` | `Demo1234!` | 登录后进入 `/me` 个人中心 |
| 管理员 | `admin@demo` | `Demo1234!` | `/admin` 后台（八类权限包，不含 `permission`） |
| 超级管理员 | `super@demo` | `Demo1234!` | `/admin` 全权限含 `permission` |

也可在登录页使用 **一键演示登录**（无需输入密码）。

> 若本地数据库在密码变更前已 seed，请删除 `server/data/chuying.db` 后重启以重新初始化。
...
已实现：CMS 首页、活动/课程、雏鹰报名与积分申请、管理后台与八类权限包、演示三角色账号密码登录与一键登录。
